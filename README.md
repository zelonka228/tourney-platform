...
![Progress](https://img.shields.io/badge/Прогрес-Тиждень%203%20з%204-yellow?style=flat-square)
...
- **Створення турніру** — назва, тип сітки, формат матчу BO1 / BO3 / BO5, дисципліна, дата; реальний вибір команд-учасників чекбоксами (з фільтром за дисципліною) з живим переглядом кількості раундів і матчів. Кнопка одразу створює турнір на бекенді й генерує сітку.
- **Турнірна сітка** — генерується на бекенді (стандартний посів, «баї» лише в 1-му раунді, автопрохід без матчу); вкладки *Сітка / Результати / Команди* показують реальні дані конкретного турніру за `/tournament/:id`.
...
- **Чемпіон і статистика** — після фіналу турнір позначається завершеним, команді-чемпіону автоматично рахується `tournaments`/`best`, обом фіналістам пишеться знімок рейтингу в `RatingHistory`.
...
- **Backend REST API** (Express + Prisma) — CRUD команд і турнірів, генерація сітки, ввід рахунку, валідація вхідних даних, єдина обробка помилок (`400` / `404` / `409`), 33 автоматизовані тести.
- **Автономний фронтенд** — якщо backend недоступний, `api.js` м'яко повертається на демонстраційні дані для перегляду; дії, що змінюють дані (створення турніру, ввід рахунку), потребують живого бекенду.

Подвійне вибування свідомо не реалізовано цього тижня (бекенд поверне `400`, а не зламається мовчки) — залишено як окремий пункт на пізніше. Далі за планом (Тиждень 4): PNG-експорт RPG-картки, фінальне тестування, звіт — див. [Статус за тижнями](#статус-за-тижнями).
...
| Тестування API | власний smoke-suite (без залежностей) | 33 сценарії happy-path + помилок |
...
backend/            Express REST API + Prisma + Socket.io
  ...
  src/routes/       teams.js, tournaments.js, matches.js
...
✅ виконано · 🔄 частково (реалізовано single elimination; double elimination — окремим пунктом пізніше) · ⏳ заплановано
