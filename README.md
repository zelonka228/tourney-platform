...
**Git-хуки** (один раз одразу після клонування, у корені репозиторію):
```bash
npm install
```
Встановлює [Husky](https://typicode.github.io/husky/) — pre-commit хук, який блокує коміт, якщо в ньому випадково опинився `.docx`, `.claude/`, `.env` чи інший файл, що не має потрапляти в репозиторій.
