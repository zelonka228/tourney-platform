...
<div align="center">
<img src=".github/assets/features.svg" alt="Ключові фічі: турнірна сітка, RPG-картки, 3D-трофей, рейтинг, ролі, локалізація" width="100%" />
</div>

Проєкт завершено (Тиждень 4/4) — усе нижче реально працює на [живому стенді](https://tourneyforge-frontend.onrender.com), не в плані.

| Категорія | Що є |
|---|---|
| Турніри | Single / double elimination (4–32 команди) · 3 режими посіву (випадковий / рейтинг / ручний) · live-рахунок через WebSocket · PNG-експорт сітки |
| Картки й колекція | RPG-картки з рідкістю Common → Legendary · паки · голо-нахил під курсором · PNG-експорт |
| Рейтинг і профілі | FACEIT ELO / MMR / звання Valorant · досягнення команди · пряме посилання на профіль · порівняння команд · обране |
| Акаунти | Ролі `admin` / `organizer` / `user` · JWT + рейт-ліміт · окрема адмін-панель для проду |
| UX | `Ctrl+K` пошук · 3D-сцена трофея чемпіону · окремий фон-мотив на кожній сторінці · 3 мови · `prefers-reduced-motion` скрізь |
| Інфраструктура | Render.com (Postgres + backend + static) · Prisma migrations · keep-alive пінг · backend smoke-suite + frontend Vitest |

Деталі — у специфікаціях: [double elimination](docs/03-double-elimination-spec.md) · [RPG-картка](docs/04-rpg-card-spec.md) · [ролі й адмінка](docs/05-auth-registration-spec.md) · [деплой](docs/06-deploy-render.md).
